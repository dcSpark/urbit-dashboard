...
function Trouble() {
  ...
  const { isConnected, hash } = useStore();

  useEffect(() => {
    let number = 0;
    if (isConnected) {
      urbitVisor.subscribe({ app: "herm", path: "/session//view" }).then((res) => {
        console.log(res, "subscribed ot herm")
        number = res.response;
  ...
  const handleHerm = useCallback((message) => {
    ...
    if (
      ...
      console.log(dojoLine, "dojo line incoming")
      if (
        !(
          dojoLine.includes("dojo>") ||
          dojoLine[0] === ";" ||
          dojoLine[0] === ">"
        )
      )
  ...
  async function handleHi() {
    setLines((previousState) => ["> Pinging ~zod..."]);
    startTerminal();
    await urbitVisor.poke({
      app: "herm",
      mark: "belt",
